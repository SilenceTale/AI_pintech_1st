...
//    }
     ...
     * @Valid 가 있으면 반드시 Errors 객체는 커맨드 객체 바로 다음에 나와야한다!
     * @return
     */
    @PostMapping("/join")
    public String joinPs(@Valid RequestJoin form, Errors errors) {
        ...
        if (errors.hasErrors()) { // 검증 실패! - reject, rejectValue가 한 번이라도 호출 되었다!
    ...
    @GetMapping("/login")
    public String login(@ModelAttribute RequestLogin form, @CookieValue(name="SavedEmail",required = false)
            String savedEmail) {
        ...
        return "member/login"; // 얘는 템플릿 경로이니 햇갈려하지 말것.
    }
